    ...
    try {
        ...
        if (data.id && data.title && data.subcategories && Array.isArray(data.subcategories)) {
            ...
            data.subcategories.forEach((subcategory: any, index: number) => {
                if (!subcategory.id || typeof subcategory.id !== 'string') errors.push(`Subcategory at index ${index} missing ID`);
                if (!subcategory.title || typeof subcategory.title !== 'string') errors.push(`Subcategory at index ${index} missing title`);
                if (!subcategory.icon || typeof subcategory.icon !== 'string') errors.push(`Subcategory at index ${index} missing icon`);
            });
            return { isValid: errors.length === 0, errors, summary };
        } else if (data.questions && Array.isArray(data.questions)) {
            summary.totalQuestions = data.questions.length;
            data.questions.forEach((question: any, index: number) => validateQuestion(question, index, errors, summary));
        } else {
