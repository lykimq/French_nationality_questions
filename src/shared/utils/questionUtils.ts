 ...
 */
export const sortQuestionsById = <T extends { id: number | string }>(
    questions: readonly T[]
): T[] => {
    return [...questions].sort((a, b) => {
        const aNum =
            typeof a.id === "number" ? a.id : (extractNumericId(a.id) ?? 0);
        const bNum =
            typeof b.id === "number" ? b.id : (extractNumericId(b.id) ?? 0);
        return aNum - bNum;
    });
};
...
const getBaseQuestionNumber = (
    question: RawQuestion | null | undefined
): number | undefined => {
...
export const processQuestionData = (
    ...
    const explanationText = ensureString(
...
export const processAllQuestions = (
    ...
    questionsData.categories.forEach(
        (category: unknown, categoryIndex: number) => {
            ...
            categoryQuestions.forEach((question: unknown) => {
                ...
                if (seenKeys.has(dedupeKey)) {
                    logger.warn(
                        `Duplicate question ID: ${questionId} in category ${categoryId}`
                    );
                    return;
                }
