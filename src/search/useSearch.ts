...
export const useSearch = () => {
    ...
    const questionTokens = useMemo(
        () => buildQuestionTokens(allQuestions),
        [allQuestions]
    );
        ...
        (query: string): SearchSuggestion[] => {
            ...
            const scoreMatch = (text: string): number => {
                ...
                for (const qt of queryTokens) {
                    if (
                        textTokens.some(
                            (tt) => tt.startsWith(qt) || qt.startsWith(tt)
                        )
                    ) {
                        tokenScore += 10;
                    }
            ...
            if (/^\d+$/.test(query)) {
                ...
                if (matchingQuestion) {
                    suggestions.unshift({
                        // Add at beginning for highest priority
                        text: `Question #${id}`,
                        type: "id",
                    });
                }
    ...
    const performSearch = useCallback(
        (query: string, appliedFilters: SearchFilters = filters) => {
            ...
            searchTimeoutRef.current = setTimeout(() => {
                ...
                const searchInQuestion =
                    appliedFilters.searchIn.includes("question") ||
                    appliedFilters.searchIn.includes("both");
                const searchInExplanation =
                    appliedFilters.searchIn.includes("explanation") ||
                    appliedFilters.searchIn.includes("both");
                ...
                if (queryId !== null) {
                    const exactMatch = filteredQuestions.find(
                        (q) => q.id === queryId
                    );
                    if (exactMatch) {
                        setSearchResults([
                ...
                const results = filteredQuestions
                    .map((item) => {
                        ...
                        // Search in question
                        if (searchInQuestion) {
                            const questionMatch = textContainsQuery(
                                questionText,
                                trimmedQuery
                            );
                    ...
                    .sort((a, b) => {
                        ...
                        const aHasExact =
                            a.matches?.some((m) => m.includes("exact")) ||
                            false;
                        const bHasExact =
                            b.matches?.some((m) => m.includes("exact")) ||
                            false;
