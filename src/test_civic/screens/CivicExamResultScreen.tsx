...
import {
    deserializeCivicExamResult,
    type SerializableCivicExamResult,
} from "../utils/civicExamSerialization";
import {
...
type CivicExamResultScreenNavigationProp =
...
const CivicExamResultScreen = () => {
    ...
    const result = serializedResult
        ? deserializeCivicExamResult(serializedResult)
        : null;
                        ...
                        >
                            {(() => {
                                const passingScore = Math.ceil(
                                    (totalQuestions *
                                        CIVIC_EXAM_CONFIG.PASSING_PERCENTAGE) /
                                        100
                                );
                                if (
                                    totalQuestions ===
                    ...
                    {incorrectQuestions.length > 0 && (
                        ...
                        >
                            ...
                            {incorrectQuestions.map((question) => {
                                const civicQuestion =
                                    question as CivicExamQuestionWithOptions;
                                const userAnswer = session.answers.find(
                                    (a) => a.questionId === question.id
                                );
                                const userAnswerIndex = parseUserAnswerIndex(
                                ...
                                const correctAnswerText =
                                    getCorrectAnswerText(civicQuestion);
