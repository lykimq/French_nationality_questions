...
const updateTopicPerformance = (
    ...
    session.questions.forEach((question) => {
        ...
        updated[topic].accuracy = Math.round(
            (updated[topic].correctAnswers /
                updated[topic].questionsAttempted) *
                100
        );
    });
...
export const updateExamStatistics = (
    ...
    if (allTimes.length > 0) {
        ...
        updated.timeStats.fastestTime = Math.min(
            updated.timeStats.fastestTime,
            ...allTimes
        );
        updated.timeStats.slowestTime = Math.max(
            updated.timeStats.slowestTime,
            ...allTimes
        );
    }
...
export const calculateProgressUpdates = (
    ...
    const newPassedExams =
        shouldUpdatePassFailStats && passed
            ? currentProgress.passedExams + 1
            : currentProgress.passedExams;
    const newFailedExams =
        shouldUpdatePassFailStats && !passed
            ? currentProgress.failedExams + 1
            : currentProgress.failedExams;

    const newTotalExamsTaken = isPracticeMode
        ? currentProgress.totalExamsTaken
        : newTotalExams;
    const newTotalPracticeSessions = isPracticeMode
        ? newTotalExams
        : currentProgress.totalPracticeSessions;
    ...
    return {
        ...
        questionsAnswered:
            currentProgress.questionsAnswered + session.totalQuestions,
        correctAnswersTotal:
            currentProgress.correctAnswersTotal + correctAnswers,
        averageScore:
            newTotalSessions > 0
                ? Math.round(
                      (currentProgress.averageScore * (newTotalSessions - 1) +
                          score) /
                          newTotalSessions
                  )
                : score,
        ...
        topicPerformance: updateTopicPerformance(
            session,
            currentProgress.topicPerformance
        ),
    };
};
