...
describe('civicExamScoring', () => {
  ...
  describe('isCivicExamPassedByCount', () => {
    it('should return true when correct answers meet passing threshold', () => {
      expect(isCivicExamPassedByCount(CIVIC_EXAM_CONFIG.PASSING_SCORE)).toBe(true);
      expect(isCivicExamPassedByCount(CIVIC_EXAM_CONFIG.PASSING_SCORE + 1)).toBe(true);
    });

    it('should return false when correct answers below threshold', () => {
      expect(isCivicExamPassedByCount(CIVIC_EXAM_CONFIG.PASSING_SCORE - 1)).toBe(false);
