 ...
 */
export const getTopicFromQuestion = (
 ...
 */
export const getSubTopicFromQuestion = (
    question: TestQuestion
): CivicExamSubTopic | null => {
...
export const getQuestionTypeFromQuestion = (
    question: TestQuestion
): QuestionType => {
...
export const filterQuestionsWithOptions = (
    questions: TestQuestion[]
): TestQuestion[] => questions.filter(hasValidOptions);

export const filterKnowledgeQuestions = (
    questions: TestQuestion[]
): TestQuestion[] =>
    questions.filter(
        (q) =>
            getQuestionTypeFromQuestion(q) === "knowledge" && hasValidOptions(q)
    );
...
const enrichQuestionWithMetadata = (
    ...
    if (!topic) {
        throw new Error(
            `Question ${question.id} missing required topic metadata`
        );
    }

    if (!subTopic) {
        throw new Error(
            `Question ${question.id} missing required subTopic metadata`
        );
    }
