...
const validateQuestionData = (
    ...
    if (!isValidId(numericId)) {
        onInvalid?.(
            `Question ${questionId || "unknown"} has invalid ID (must contain a positive number)`
        );
        return false;
    }
    ...
    if (!hasValidQuestionType) {
        onInvalid?.(
            `Question ${questionId} has invalid questionType: "${question.questionType}" (must be "knowledge" or "situational")`
        );
        return false;
    }

    const theme = question.theme as string | undefined;
    const hasValidTopic = theme
        ? isValidTopic(theme)
    ...
    if (!hasValidTopic) {
        ...
        onInvalid?.(
            `Question ${questionId} has invalid topic: ${themeStr} (must be one of: ${ALL_TOPICS.join(", ")})`
        );
        return false;
    }
    ...
    if (!isValidSubTopic(subTheme)) {
        onInvalid?.(
            `Question ${questionId} has invalid subTopic: "${subTheme}" (must be one of: ${VALID_SUBTOPICS.join(", ")})`
        );
        return false;
    }
    ...
    if (isKnowledgeQuestion) {
        ...
        if (!isNonEmptyString(correctAnswer)) {
            onInvalid?.(
                `Question ${questionId} (knowledge type) missing or empty correctAnswer`
            );
            return false;
        }
        ...
        if (!Array.isArray(incorrectAnswers) || incorrectAnswers.length === 0) {
            onInvalid?.(
                `Question ${questionId} (knowledge type) missing or empty incorrectAnswers array`
            );
            return false;
        }

        const validIncorrectAnswers = incorrectAnswers.filter((ans: unknown) =>
            isNonEmptyString(ans)
        );
        if (validIncorrectAnswers.length === 0) {
            onInvalid?.(
                `Question ${questionId} (knowledge type) incorrectAnswers array contains no valid strings`
            );
            return false;
        }
    }
 ...
 */
const resolveCorrectAnswerIndex = (
...
const transformCivicQuestion = (
    ...
    const correctExplanationIndex = validateAnswerIndex(
        q.correctExplanationAnswer,
        explanationOptions.length
    );
    ...
    return {
        ...
        image:
            typeof q.image === "string" && q.image.trim().length > 0
                ? q.image.trim()
                : undefined,
...
const loadQuestionsFromFileData = (
    ...
    if (Array.isArray(fileData)) {
        ...
        if (fileData.themeId) {
            ...
            if (isValidTopic(fileData.themeId)) {
                ...
                logger.warn(
                    `Invalid topicId (themeId in JSON) in file: "${fileData.themeId}"`
                );
            }
        }
    } else {
    ...
    if (!topic) {
        logger.warn(
            "Could not determine topic for civic exam questions, skipping"
        );
        return [];
    }
    ...
    if (
        ...
        logger.warn(
            `Skipped ${questions.length - validQuestions.length} invalid question(s): ${errorSummary}`
        );
    }
...
export const loadCivicExamQuestions = async (): Promise<
    CivicExamQuestionWithOptions[]
> => {
    try {
        ...
        if (principesData) {
            allQuestions.push(
                ...loadQuestionsFromFileData(
                    principesData as CivicExamDataFile | CivicExamQuestionData[]
                )
            );
        }
        ...
        if (systemData) {
            allQuestions.push(
                ...loadQuestionsFromFileData(
                    systemData as CivicExamDataFile | CivicExamQuestionData[]
                )
            );
        }
        ...
        if (droitsData) {
            allQuestions.push(
                ...loadQuestionsFromFileData(
                    droitsData as CivicExamDataFile | CivicExamQuestionData[]
                )
            );
        }
        ...
        if (histoireData) {
            allQuestions.push(
                ...loadQuestionsFromFileData(
                    histoireData as CivicExamDataFile | CivicExamQuestionData[]
                )
            );
        }
        ...
        if (vivreData) {
            allQuestions.push(
                ...loadQuestionsFromFileData(
                    vivreData as CivicExamDataFile | CivicExamQuestionData[]
                )
            );
        }
